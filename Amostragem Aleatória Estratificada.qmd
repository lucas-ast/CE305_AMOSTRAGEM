...
#### [Aula 01 - Amostragem Aleatória Estratificada Simples](https://youtu.be/4MD_3-5wsno?si=4REOOJ8_-uYZEKhj)
...
#### [Aula 02](https://youtu.be/3NJADJL4Svo?si=R85zW6jfPnBjBdbF)
...
#### [Aula 03](https://youtu.be/M0MDqW2umlI?si=AGZcT0-_mvv3B76H)
...
#### [Aula 04](https://youtu.be/DeVMl89thg4?si=70GcKmHQk4L99KJf)
...
-   Variância da média estratificada: $$
    \hat{V}[\bar{y}_{st}] = \sum \frac{W_h^2 S_h^2}{n_h}
    $$

-   Intervalo de confiança para $\bar{y}_{st}$: $$
    \bar{y}_{st} \pm t \sqrt{\hat{V}[\bar{y}_{st}]}
    $$

-   Alocação ótima por estrato: $$
    n_h = \eta \cdot \frac{W_h S_h / \sqrt{C_h}}{\sum_{i=1}^L W_i S_i / \sqrt{C_i}}
    $$
...
| Estrato | $N_h$ | $S_h$ | $C_h$ | $W_h S_h \sqrt{C_h}$ | $W_h S_h / \sqrt{C_h}$ | $W_h^2 S_h^2 / N_h$ |
|-----------|-----------|-----------|-----------|-----------|-----------|-----------|
| 1 | 5000 | 10 | 4 | 1,25 | 1,25 | 0,00125 |
| 2 | 8000 | 8 | 9 | 1,067 | 1,067 | 0,002125 |
| 3 | 3000 | 10 | 16 | 0,375 | 0,375 | 0,000278 |
| 4 | 4000 | 9 | 4 | 0,90 | 0,90 | 0,00081 |
| **Total** | 20000 | — | — | **3,592** | **3,592** | **0,004463** |

------------------------------------------------------------------------

##### ✅ Cálculo do tamanho amostral total:

$$
\eta = \frac{(3,5917) \cdot (24,2)}{0,065 + 0,004409} \approx \frac{86,90914}{0,069409} \approx \boxed{1252}
$$
...
$$
\text{Custo total} = C_0 + 437 \cdot 4 + 373 \cdot 9 + 131 \cdot 16 + 315 \cdot 4 = 8461
$$
...
#### Aula 01 – Conceito de Alocação Ótima

##### Objetivo da aula

Definir o conceito de alocação ótima e mostrar as três abordagens possíveis:

1. **Alocação Proporcional**:

   * Tamanho da amostra no estrato proporcional ao tamanho da população no estrato:
     $n_h = n \cdot \frac{N_h}{N}$

2. **Alocação de Neyman (ou ótima em termos de variância)**:

   * Considera o tamanho e a variabilidade dentro de cada estrato:
     $n_h = n \cdot \frac{N_h S_h}{\sum N_h S_h}$

3. **Alocação ótima considerando custos**:

   * Leva em conta o custo por unidade em cada estrato:
     $n_h = n \cdot \frac{N_h S_h / \sqrt{C_h}}{\sum N_h S_h / \sqrt{C_h}}$

---

#### Aula 02 – Controlando o Custo

##### Objetivo da aula

Controlar o **custo total** da amostragem estratificada, determinando o \*\*tamanho total da amostra **$n$** dado um orçamento fixo $C$.

Fórmula:

$$
\eta = \frac{(C - C_0) \cdot \sum_{h=1}^{L} N_h S_h / \sqrt{C_h}}{\sum_{h=1}^{L} N_h S_h \sqrt{C_h}}
$$

Onde:

* $C$: orçamento total disponível,
* $C_0$: custo fixo (piloto, estrutura etc.),
* $C_h$: custo por unidade no estrato $h$,
* $S_h$: desvio padrão do estrato $h$,
* $N_h$: tamanho da população no estrato $h$.

Com $n$ obtido pela fórmula, alocamos as amostras por:
...
#### Aula 03 – Controlando a Precisão na Amostragem Aleatória Estratificada

##### Objetivo da aula

Controlar a **precisão** da estimativa da média populacional $\bar{Y}_{st}$, determinando o \*\*tamanho ótimo da amostra total **$n$**.

### Fórmula para controle da precisão

$$
\eta = \frac{
\left( \sum W_h S_h \sqrt{C_h} \right)\left( \sum \frac{W_h S_h}{\sqrt{C_h}} \right)
}{
V + \sum \frac{W_h^2 S_h^2}{N_h}
}
$$

Onde:

* $W_h = \frac{N_h}{N}$: fração do estrato $h$,
* $S_h$: desvio padrão do estrato $h$,
* $C_h$: custo de observação no estrato $h$,
* $V = \left( \frac{E}{z} \right)^2$: precisão desejada (erro ao quadrado sobre valor crítico da normal),
* $N_h$: tamanho da população no estrato $h$.
